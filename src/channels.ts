...
import { assertVersionSelector as assertProtocolVersionSelector, negotiateVersion } from './versioning.js';
...
function assertChannelVersionSelector(version: string): void {
  try {
    assertProtocolVersionSelector(version);
  } catch {
...
export function validateChannelDeclaration(declaration: ChannelDeclaration): void {
  ...
  for (const version of declaration.versions) {
    ...
    assertChannelVersionSelector(version);
  }
...
export class ChannelForger {
  ...
  resolve(
    ...
    for (const version of requiredVersions) assertChannelVersionSelector(version);
